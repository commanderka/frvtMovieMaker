...
```python
    ...
    algoInfo1 = AlgorithmInfo("sampleAlgo_001",os.path.join(baseDir,"edbs"),implDir = baseDir,libName = "libfrvt_1N_sampleAlgo_001.so", enrollmentDir=os.path.join(baseDir,"enroll"))
...
## Databases used
Two background databases containing 10k and 1M distractors (publically available celebrity images) were used for performing the 1:N search.

### Sample Movies
[<img src="sampleMovie/output.gif">](https://youtu.be/MerOzYzxRxM)


A generated movie using a sample face recognition algorithm and two different edbs (containg templates of random celebrities) can be found under sampleMovies.
...
* Provide method to show all gitlist images (if thumbnails are provided)
